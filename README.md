...
| **정정 공시 이력** | `list.json` 비고(`rm`)의 정정·철회 신호와 보고서 식별자를 결합한 원본-정정본 계보, 최신 유효본 및 철회 상태 판별 | 조회 범위 밖 원본이나 모호한 외부 계보처럼 OpenDART 메타데이터로 유일하게 연결할 수 없는 이력 |
| **공시 이벤트** | 공시 목록상 주요 보고서 접수 현황, `rm` 기반 철회 상태 및 원문에 명시된 사건 사실 | 배당·임원보수·주요사항보고서 등의 전용 정형 API가 필요한 금액·발생일자 |
...
Streamlit UI에서 일반 질문과 후속 질문을 대화 형태로 입력할 수 있습니다. 후속 질문은 현재 기업·연도·지표·연결/별도 문맥을 반영한 독립 질의로 변환된 뒤 동일한 `/answer` 검증 경로를 통과합니다.

백엔드 서버를 실행한 상태에서 다른 터미널로 UI를 시작합니다.
...
브라우저에서 `http://127.0.0.1:8600`을 열고 다음 순서로 사용합니다.

1. 백엔드 모드를 `실제 서버`로 선택합니다.
2. 서버 주소를 `http://127.0.0.1:8001`로 설정하고 `연결 확인`을 누릅니다.
3. 기업·연도·지표가 포함된 질문을 입력합니다.
4. 답변의 DART 링크 또는 `공시 근거 보기`에서 접수번호, 문서 상태, 원문 표를 확인합니다.
5. `영업이익은?`처럼 짧은 후속 질문을 입력해 적용된 문맥과 실제 전송 질의를 확인합니다.

### 1. 실제 서버 연결

![실제 서버 연결 확인](docs/assets/ui/ui-01-live-connection.jpg)

### 2. 계산 결과와 공시 인용

![전년 대비 증가율 답변과 사업보고서 인용](docs/assets/ui/ui-02-grounded-answer.jpg)

### 3. 원문 근거 상세

![접수번호와 연결 손익계산서 원문 근거](docs/assets/ui/ui-03-evidence-detail.jpg)

### 4. 후속 질문 문맥 반영

![후속 질문을 독립 질의로 변환한 화면](docs/assets/ui/ui-04-context-followup.jpg)

위 화면은 실제 OpenDART 서버에 연결해 삼성전자 2023·2024 사업보고서를 조회한 결과입니다. 자세한 사용법과 모의 모드는 [대화형 UI 사용 안내](docs/CHAT_UI_USAGE.md)를 참고하세요.
